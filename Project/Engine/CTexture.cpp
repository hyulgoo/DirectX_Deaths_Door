...
void CTexture::Clear_CS(bool _bShaderRes) const
{
	if (_bShaderRes)
	{
		ID3D11ShaderResourceView* pSRV = nullptr;
...
void CTexture::UpdateData() const
{
}
...
{
	// 기존에 로딩된 텍스쳐를 초기화하고 인자로 들어온 만큼의 밉맵을 가진 텍스쳐로 다시 로딩할 것
	// 상용엔진은 초기화가 아닌 실시간으로 밉맵을 만들어 넣어주는 방식을 사용함.
	// 여기서는 그 방식이 복잡하므로 초기화 후 재생성 방식을 사용함.
	m_Tex2D = nullptr;
	m_SRV = nullptr;
	m_RTV = nullptr;
	m_DSV = nullptr;
...
{
	// 텍스쳐를 파일로 저장하는 기능
	// Cube의 경우 dds, 일반 텍스쳐의 경우 png로 저장됨// 경로
	...
	if (1 < m_Desc.MipLevels)
	{
		...
		for (int i = 0; i < static_cast<int>(m_Desc.ArraySize); ++i)
		{
			// m_Tex2D ==> pCopyTex2D

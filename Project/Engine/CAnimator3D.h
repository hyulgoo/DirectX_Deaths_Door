...
public:
    ...
    int  GetCurClip() const { return m_iCurClip; }                                    // 현재 Cilp 인덱스를 반환하는 함수.
    int  GetClipSize() const { return static_cast<int>(m_vecClipUpdateTime.size()); } // 전체 Clip 개수를 반환하는 함수.
    ...
    const vector<tMTAnimClip>* GetAnimClip() const { return m_pVecClip; } // 현재 애니메이션 클립 정보 Vecotr를 반환하는 함수.
